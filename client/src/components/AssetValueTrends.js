...
const AssetValueTrends = () => {
    ...
    const fetchCategoryAssetValues = async () => {
      ...
      try {
        const response = await api.get(
          `/asset-values/category/${selectedCategory}`
        );
  ...
  const calculateTotalChange = () => {
    ...
    const lastValue =
      weeklyAssetValues[weeklyAssetValues.length - 1].totalAssetValue;
        ...
        <div className="flex items-center space-x-4">
          ...
          <div
            className={`flex items-center ${
              calculateTotalChange() >= 0 ? "text-green-600" : "text-red-600"
            }`}
          >
